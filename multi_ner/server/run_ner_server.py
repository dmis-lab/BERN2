...
from typing import Any, Callable, Dict, List, Optional, Tuple, Union, NewType, NamedTuple
...
def convert_examples_to_features(
    ...
    for (ex_index, example) in tqdm(enumerate(examples)):
        ...
        write_tokens(tokens, det_tokens, 'test', req_id)
    return features

def write_tokens(tokens, det_tokens, mode, req_id):
    if mode == "test":
        ...
        det_path = os.path.join("multi_ner", "tmp",
                            "det_token_{}_{}.txt".format(mode, req_id))
...
class BioBERT:
    def __init__(self, params):
        ...
        # Prepare Labels
        self.labels = self.processor.get_labels(self.params.labels)
        ...
        # self.model = MultiNER2.from_pretrained(
        self.model = RoBERTaMultiNER2.from_pretrained(
            self.params.model_dir,
            from_tf=bool(".ckpt" in self.params.model_dir),
            num_labels=self.num_labels,
            config=self.config,
            cache_dir=self.params.cache_dir,
        )
    ...
    @Profile(__name__)
    def recognize(self, input_dl, is_raw_text=False, thread_id=None, indent=None):
        # same as bern
        ...
        det_token_path = os.path.join("multi_ner", "tmp",
                                  "det_token_test_{}.txt".format(req_id))
        ...
        predict_example_list = (NerDataset(predict_examples, self.labels,\
                                self.tokenizer, self.config, self.params, req_id))
        ...
        self.predict_dict, self.logits_dict = dict(), dict()
        ...
        data_list = merge_results(self.data_list, self.json_dict, self.predict_dict,
                                  self.logits_dict, self.params.rep_ent,
                                  is_raw_text=is_raw_text, entity_types=self.entity_types)
        ...
        if type(input_dl) is str:
            ...
            with open(output_path, 'w') as resultf:
                for paper in data_list:
                    paper['ner_model'] = "BioBERT NER v.20210707"
                    resultf.write(
    ...
    def recognize_etype(self, etype, tokens, tot_tokens, predictions, preds_array):
        ...
        self.logits_dict[etype] = dict()
        piv = 0
        for data in self.data_list:
            pmid = data['pmid']
            self.predict_dict[etype][pmid] = list()
            self.logits_dict[etype][pmid] = list()
            ...
            while True:
                if overlen:
                    try:
                        ...
                        break
                    self.logits_dict[etype][pmid][-1].extend(de_logits[piv + de_i])
                    de_i += 1
                    ...
                    self.logits_dict[etype][pmid].append(de_logits[piv + de_i])
                    de_i += 1
                    if len(self.predict_dict[etype][pmid][-1]) == len(
                            self.json_dict[pmid]['words'][
                                len(self.predict_dict[etype][pmid]) - 1]):
    ...
    def _prediction_loop(
        ...
        for inputs in tqdm(dataloader, desc=description):
            ...
            if not prediction_loss_only:
                ...
                if dise_preds is None and chem_preds is None and gene_preds is None and spec_preds is None and cl_preds is None and dna_preds is None and rna_preds is None and ct_preds is None:
                    ...
                    # protein_preds = protein_logits.detach()
                    ...
                    # protein_preds = torch.cat((protein_preds, protein_logits.detach()), dim=0)
        ...
        if dise_preds is not None and chem_preds is not None and gene_preds is not None and spec_preds is not None:
            ...
            # protein_preds = protein_preds.cpu().numpy()
        ...
        return_output = (PredictionOutput(predictions=dise_preds, label_ids=label_ids), \
                        ...
                        # PredictionOutput(predictions=protein_preds, label_ids=label_ids), \
                        PredictionOutput(predictions=ct_preds, label_ids=label_ids))
                        
        return return_output
        
    def compute_metrics(self, p: EvalPrediction) -> List[int]:
        preds_list = self.align_predictions(p.predictions)
        return preds_list

    # def align_predictions(self, predictions: np.ndarray) -> Tuple[List[int], List[int]]:
...
def main():
    ...
    argparser.add_argument('--gnormplus_home',
                            help='GNormPlus home',
                            default=os.path.join(os.path.expanduser('~'), 'bern', 'GNormPlusJava'))
    ...
    argparser.add_argument('--tmvar2_home',
                            help='tmVar 2.0 home',
                            default=os.path.join(os.path.expanduser('~'), 'bern', 'tmVarJava'))
    ...
    argparser.add_argument('--model_dir',
                            help='pretrained model directory', default='./resources/multi_ner/')
    argparser.add_argument('--bert_config_file',
                            help='pretrained model config file directory', default='./resources/multi_ner/config.json')
    argparser.add_argument('--vocab_file',
                            help='pretrained model vocab file directory', default='./resources/multi_ner/vocab.txt')
    argparser.add_argument('--init_checkpoint',
                            help='pretrained model init checkpoint directory', default='./resources/multi_ner/pytorch_model.bin')
    argparser.add_argument('--config_name',
                            help='Pretrained config name or path if not the same as model_name', default='./resources/multi_ner/')
    argparser.add_argument('--tokenizer_name',
                            help='Pretrained tokenizer name or path if not the same as model_name', default='./resources/multi_ner/')
    argparser.add_argument('--cache_dir', 
                            help='Where do you want to store the pretrained models downloaded from s3', default='./cached/')
    argparser.add_argument('--labels',
                            help='Path to a file containing all labels', default='./resources/multi_ner/labels.txt')
    ...
    argparser.add_argument('--overwrite_cache', action='store_true', 
                            help='Overwrite the cached training and evaluation sets', default=False)
    argparser.add_argument('--data_list', 
                            help='List of entity name string', default='')
    argparser.add_argument('--eval_data_type', 
                            help='Entity name or Entity type for evaluation', default='')
    argparser.add_argument('--seed', type=int, help='random seed for initialization',
                            default=1)
    argparser.add_argument('--rep_ent', action='store_true', 
                            help='Off entity type decision rules? Whether to print all rep outputs', default=False)
    args = argparser.parse_args()

    biobert = BioBERT(args)
